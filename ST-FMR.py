...
def output_method(control_dict, mag_dict, lockin_dict):
    ...
    if output.lstrip('-').replace('.','',1).isdigit():
        ...
        if float(output) / float(interval) < float(control_dict['Hx DAC Limit']):
            amp.dacOutput((float(output) / float(interval)), control_dict['%s DAC Channel'])
            time.sleep(float(t))
            amp.dacOutput(0, control_dict['%s DAC Channel'])
    ...
    def measure_loop():
        ...
        if max(scan_field_output) / float(control_dict['Hx/DAC (Oe/V)']) < float(control_dict['Hx DAC Limit']):
            ...
            for counter, scan_val in enumerate(scan_field_output):
                ...
                # save data
                save_method(scan_field_output, measured_values, display, control_dict['Directory'], control_dict['File Name'].get(), lockin_dict)
