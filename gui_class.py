...
def animate(i):
    ...
    ax.clear()
    ax.plot(scan_field_output, measured_values)
    dataplot.draw()

# class that builds the whole GUI framework
class Measureapp(tk.Tk):

    def __init__(self, *args, **kwargs):

        tk.Tk.__init__(self, *args, **kwargs)
        tk.Tk.wm_title(self, "AHE/SMR Measurement")
        ...
        plt_frame = Frame(container, borderwidth=10, relief="sunken")
        settings_frame = Frame(container, borderwidth=5)
        information_frame = Frame(container, borderwidth=5)
        buttons_frame = Frame(container, borderwidth=5)
        rows =20
        ...
        # initialize all frames that display info/data
        control_dict['Display'] = self.make_info(information_frame)
        mag_dict = self.make_form(settings_frame, mag_dict, 'Magnetic Settings')
        keith_dict = self.make_form(settings_frame, keith_dict, 'Current Settings')
        self.make_extras(settings_frame, mag_dict, keith_dict, control_dict)
        self.make_plot(plt_frame, plot_title, x_lbl, y_lbl)
        self.make_buttons(buttons_frame, mag_dict, keith_dict, control_dict, plot_title, x_lbl, y_lbl)

        #weights columns for all multiple weight=1 columns
        self.weight(buttons_frame)
        self.weight(information_frame)
        self.weight(settings_frame)
        ...
        control_dict['Directory'] = self.set_directory(control_dict['Display'])

    # takes a given frame and gives all columns a weight of 1
    def weight(self, frame):
    ...
    def make_form(self, root, dictionary, frametxt):
    ...
    def make_plot(self, root, title, x_label, y_label):
    ...
    def make_extras(self, root, mag_dict, keith_dict, control_dict):
    ...
    def make_buttons(self, root, mag_dict, keith_dict, control_dict, plot_title, x_lbl, y_lbl):
    ...
    def set_directory(self, display):
